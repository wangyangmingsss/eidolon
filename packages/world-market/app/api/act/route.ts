...
import { buildMarketContext, TASKS } from '@/lib/market-config';
...
export async function POST(req: NextRequest) {
  ...
  if (!tokenId || !taskId || !userInput) return new Response('Missing fields', { status: 400 });
  ...
  try {
    ...
    const ctx = buildMarketContext(taskId);
    ...
    return Response.json({
      ...
      soul: JSON.parse(JSON.stringify(updatedSoul, (_, v) =>
